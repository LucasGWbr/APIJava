...
public interface eventRepository extends JpaRepository<Events, Integer> {
    @Query("SELECT new br.univates.api.dtos.eventInscriptionDTO(e.name,e.description,e.startDate,e.endDate,e.location,e.certificateTemplatePath, i.inscriptionId, e.eventId) FROM inscription i JOIN Events e ON e.eventId = i.eventId JOIN users u ON u.user_id = i.userId WHERE u.user_id = :userId AND i.status = 'ACTIVE'")
